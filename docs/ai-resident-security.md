...
1. Issue #61の投稿後自動観測では、投稿作成成功後に `POST /api/ai-observation-auto-request` を裏側で呼ぶ。通常UIにはボタンやstatusを出さず、通常対象は投稿者本人の `public text` 投稿とする。operatorまたは `username = 'hoshizora_hoshikun'` 限定は解除する。通常jobは `observation_context = 'auto_text_post'`、初投稿歓迎jobは `first_post_welcome` と `not_before_at` を持ち、scheduled Functionがdue jobだけをworkerへdispatchする。通常星文はモデル判断、confidence、70%確率、日次上限、投稿者単位クールダウンで抑制する。投稿形式を問わない最初の公開・未削除流星便だけは、service_role専用の `chia_first_post_welcomes` 記録をtransaction内で確定し、text / image / video / youtubeを対象に確率・confidence・日次星文上限・投稿者クールダウンをバイパスして一度だけ歓迎星文を作る。通常の2件目以降の画像・動画・YouTubeを自動観測へ広げず、単体音声は従来どおりfail closedとする。validator/schema、危険時のfail closed、料金上限やjob上限の挙動は変更しない。
