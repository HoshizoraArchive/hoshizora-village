...
1. Issue #61の投稿後自動観測では、投稿作成成功後に `POST /api/ai-observation-auto-request` を裏側で呼ぶ。通常UIにはボタンやstatusを出さず、対象は投稿者本人の `public text` 投稿すべてに広げる。operatorまたは `username = 'hoshizora_hoshikun'` 限定は解除する。自動観測jobは `observation_context = 'auto_text_post'` と `not_before_at` を持ち、scheduled Functionがdue jobだけをworkerへdispatchする。自動観測由来のtext jobでは、観測結果を `public.observations` に保存し、ちあ名義の `public.resonances` を1件作成する。通常星文はモデル判断、confidence、70%確率、日次上限、投稿者単位クールダウンで抑制する。最初の公開・未削除投稿だけは、service_role専用の `chia_first_post_welcomes` 記録をtransaction内で確定し、確率・日次上限・投稿者クールダウンをバイパスして一度だけ歓迎星文を作る。validator/schema、危険時のfail closed、上限到達時のfail/cancelは変更しない。
