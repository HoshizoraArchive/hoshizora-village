...
The SQL now uses `pg_catalog` ACL inspection with `aclexplode` for table/view/sequence/schema/function privileges so `PUBLIC`, `anon`, and `authenticated` grants are visible. Function EXECUTE checks use `pg_proc.proacl` and `aclexplode`, not `has_function_privilege('public', ...)`. `acldefault()` calls cast the object-type argument to `"char"` explicitly to avoid environment-dependent function resolution. The browser write-grant anomaly section includes `PUBLIC` grants because they are effective for `anon` and `authenticated`. It also includes default privileges, a public schema object inventory, and explicit RLS anomaly sections.
