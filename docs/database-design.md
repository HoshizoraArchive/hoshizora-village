...
- `observation_context`: `manual`、通常のtext投稿後自動観測 `auto_text_post`、初公開流星便歓迎 `first_post_welcome`
...
- `observation_context` は `manual` / `auto_text_post` / `first_post_welcome` のみ
...
- `public.chia_first_post_welcomes` は投稿者ごとの初投稿歓迎を一度だけ残す内部記録。`app_private.is_chia_first_public_post(...)` は、投稿形式を限定せず同じ投稿者の公開・未削除投稿を `(created_at, id)` で比較するため、非公開・削除済み投稿は初公開流星便を妨げない。現在の自動観測対象は `text` / `image` / `video` / `youtube` である
- `public.reserve_chia_first_post_welcome_job(...)` は投稿者単位advisory lockの中で共通helperを再確認してから既存予約RPCへ委譲する。completion RPCも同じhelperで再判定し、初投稿だけは通常の確率・confidence・日次星文上限・投稿者クールダウンをバイパスして、星文・歓迎記録・job完了を同一transactionで確定する。投稿削除後も歓迎済みの `author_id` は残る
