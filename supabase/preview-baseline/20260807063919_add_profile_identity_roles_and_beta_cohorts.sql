...
-- DDL reconstructed from Production migration 20260807063919.
-- Production profile/auth backfills and named-account assignments are omitted.
...
create policy profile_kinds_select_public
  ...
  using (
    exists (
      ...
      where p.id = profile_kinds.profile_id
    )
  );
...
create policy profile_cohorts_select_public
  ...
  using (
    exists (
      select 1
      from public.profiles p
      where p.id = profile_cohorts.profile_id
    )
  );
