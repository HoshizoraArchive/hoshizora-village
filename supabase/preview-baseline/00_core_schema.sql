...
-- Sanitized clean-target baseline for Re:AiSNS / 星空Village.
-- Contains schema and non-secret catalog rows only. It contains no Auth users,
-- user-generated rows, Storage objects, credentials, or Production UUIDs.
-- Apply only in the README order to an empty Supabase project after approval.
...
grant all on table public.profile_frame_ownerships to service_role;
...
-- Profile-specific title assignment is intentionally excluded. Preview identities
-- must be created synthetically after this baseline and assigned explicitly.
