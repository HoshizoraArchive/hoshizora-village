...
test("YouTube and uploaded 星映 keep the chosen transparency and brightness without a pasted-on frame", () => {
  ...
  assert.match(cssSource, /@media \(max-width: 1023px\) and \(orientation: landscape\)[\s\S]*?\.post-video-youtube > \.star-movie-surface\s*\{[\s\S]*?transform: translateX\(-2%\) scale\(1\.14\)/);
